 ...
 *
 * Vanilla JS, no build step, no framework, no CDN. Loaded as an ES module
 * (`<script type="module" src="static/app.js">` in index.html):
 *   * the pure helpers (formatBytes, parseAddress) are imported from the
 *     sibling `lib.js` module — a tiny testability extraction (issue #26,
 *     PR 2; Q2 approved in the issue thread) so the vitest unit suite can
 *     test them in isolation;
 *   * the vendored esptool-js 0.6.1 browser bundle is loaded with a
 *     dynamic `import()` resolved relative to *this* module
 *     (import.meta.url → `static/vendor/esptool-js.bundle.js`). The bundle
 *     is an ES module (verified against the esptool-js 0.6.1 npm tarball);
 *     it exposes `ESPLoader` and `Transport` as named exports, which we
 *     destructure and use directly.
 *
 * Module scope means no globals are leaked (same guarantee the former IIFE
 * provided).
 *
...
import { formatBytes, parseAddress } from "./lib.js";
...
// ------------------------------------------------------------------
// Selection helpers (formatBytes / parseAddress now come from lib.js)
// ------------------------------------------------------------------
...
async function loadBundle() {
  ...
  try {
    ...
    // Resolve it relative to *this* module (a sibling vendor/ dir), which
    // is independent of how the page is mounted (root, sub-path,
    // reverse-proxy prefix).
    const bundleUrl = new URL("vendor/esptool-js.bundle.js", import.meta.url).href;
